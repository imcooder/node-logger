...
module.exports = class Tracer {
    ...
    debug(format, ...args) {
        if (!this.logger || this.logger.getIntLevel() > 16) {
            return;
        }
    ...
    info(format, ...args) {
        if (!this.logger || this.logger.getIntLevel() > 4) {
            return;
        }
        format = 'logid:%s ' + format;
        args = Tracer.serilize(args);
    ...
    warn(format, ...args) {
        if (!this.logger || this.logger.getIntLevel() > 2) {
            return;
        }
        format = 'logid:%s ' + format;
        args = Tracer.serilize(args);
    ...
    error(format, ...args) {
        if (!this.logger || this.logger.getIntLevel() > 1) {
            return;
        }
        format = 'logid:%s ' + format;
        args = Tracer.serilize(args);
